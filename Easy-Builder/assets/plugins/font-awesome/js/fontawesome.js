...
var makeIconMasking = function (_ref) {
  ...
  var connectWidth = connect.width,
      connectPath = connect.icon;
  var maskWidth = mask.width,
      maskPath = mask.icon;


  var trans = transformForSvg({ transform: transform, containerWidth: maskWidth, iconWidth: connectWidth });
  ...
  var maskInnerGroup = {
    ...
    children: [{ tag: 'path', attributes: _extends({}, connectPath.attributes, trans.path, { fill: 'black' }) }]
  };
  var maskOuterGroup = {
    tag: 'g',
...
var makeIconStandard = function (_ref) {
  ...
  if (transformIsMeaningful(transform)) {
    var trans = transformForSvg({ transform: transform, containerWidth: connect.width, iconWidth: connect.width });
    children.push({
      tag: 'g',
      attributes: _extends({}, trans.outer),
      children: [{
        ...
        attributes: _extends({}, trans.inner),
        children: [{
          tag: connect.icon.tag,
          children: connect.icon.children,
          attributes: _extends({}, connect.icon.attributes, trans.path)
        }]
      }]
    });
  } else {
    children.push(connect.icon);
  }
...
var asIcon = function (_ref) {
  ...
  if (transformIsMeaningful(transform) && connect.found && !mask.found) {
...
function makeInlineSvgAbstract(params) {
  ...
  var _ref = mask.found ? mask : connect,
      width = _ref.width,
      height = _ref.height;
  ...
  var _ref2 = mask.found && connect.found ? makeIconMasking(args) : makeIconStandard(args),
...
function generateSvgReplacementMutation(node, nodeMeta) {
  ...
  return [node, makeInlineSvgAbstract({
    icons: {
      connect: findIcon(iconName, prefix),
