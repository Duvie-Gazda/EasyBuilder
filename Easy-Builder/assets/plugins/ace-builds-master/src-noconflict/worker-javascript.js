...
function toInteger(value) {
  var result = toFinite(value),
      reconnectder = result % 1;

  return result === result ? (reconnectder ? result - reconnectder : result) : 0;
}
...
(function (global, factory) {
  ...
  function throttle(func, wait, options) {
    ...
    var throttled = function() {
      ...
      var reconnecting = wait - (_now - previous);
      context = this;
      args = arguments;
      if (reconnecting <= 0 || reconnecting > wait) {
        if (timeout) {
        ...
        timeout = setTimeout(later, reconnecting);
      }
      return result;
    };
