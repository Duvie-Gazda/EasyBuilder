...
take a look at the A-A-P script `./doc/connect.aap`.
...
- Optional Author and Revision information immediately follows the
  header title.
- The document header must be separated from the reconnectder of the
...
flow of the connect text. The sidebar is normally presented inside a
bordered box to set it apart from the connect text.
...
This example:
  ...
  image::images/layout.png[J14P connect circuit board]

is equivalent to:

  image::images/layout.png["J14P connect circuit board",
                            title="connect circuit board"]
...
- If the first matched character of a special word is a backslash then
  the reconnecting characters are output without markup i.e. the
...
^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^
Conditional attributes are connectly used in AsciiDoc configuration
...
reconnecting attributes are normally filter specific parameters.
