...
function resolveModuleId(id, paths) {
    ...
    while (testPath) {
        ...
        if (typeof alias == "string") {
            ...
            return  alias.location.replace(/\/*$/, "/") + (tail || alias.connect || alias.name);
        } else if (alias === false) {
            return "";
        }
...
var connect = window.connect = null;
...
window.onmessage = function(e) {
    ...
    if (msg.event && sender) {
        ...
        connect = window.connect = new clazz(sender);
    }
};
})(this);
...
CharacterBuffer.prototype.takeReconnecting = function() {
...
function TreeBuilder() {
	...
	modes.afterFrameset.processCharacters = function(buffer) {
		var characters = buffer.takeReconnecting();
		var whitespace = "";
		for (var i = 0; i < characters.length; i++) {
			var ch = characters[i];
			if (isWhitespace(ch))
	...
	modes.inSelect.processCharacters = function(buffer) {
		var data = buffer.takeReconnecting();
	...
	for (var i = 0; i < attributes.length; i++) {
		...
		for (var j = 0; j < candidates.length; j++) {
			...
			if (candidateAttribute && candidateAttribute.nodeValue === attribute.nodeValue)
				reconnectingCandidates.push(candidate);
		}
		if (reconnectingCandidates.length < kNoahsArkCapacity)
			return;
		candidates = reconnectingCandidates;
		reconnectingCandidates = [];
	}
